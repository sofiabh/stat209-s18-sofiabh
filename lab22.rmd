...
**Answer**: The 3rd is 369.00 and the 97th is 4107.46
...
**Answer**: The perfectionism score is lower for the fast variable, but there isn't much difference between normal and slow.  Both are significant because they don't contain zero.
...
**Answer**: A reason some people may finish quickly is that they read quicker.  A reason some people may take a long time is a higher percentage of the trait "perfectionism" influencing how seriously they take the quiz.
...
```{r}
model <- lm_basic(warmth ~ 1 + sensitivity + dominance + baseline, data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**: The slope for dominance is not statistically significant, but sensitivity is.  Despite this, I would have expected the results because the variables are not that different from each other and there is not much change.

Take the previous model and add the variables `speed`, `gender`, and
`country`, the latter lumped into 5 categories.

```{r}
model <- lm_basic(warmth ~ 1 + sensitivity + dominance + baseline + speed + gender + fct_lump(country, 5), data = pf)
reg_table(model, level = 0.95)
```

Does this change the slopes for your two traits much?

**Answer**: Yes, it changes it slightly.

Summarize the previous model (specifically the relationship between the
three traits) in words.

**Answer**: In controlling for the variables of speed, gender, and country, we can see a more accurate prediction of the relationship between the scores of the chosen traits (wartmh, dominance, and sensitivity) without having the scores influenced by the other factors.
